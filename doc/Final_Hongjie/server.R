...
people <- data.frame(STATE=map_data$STATE, INJUR_DEATH=map_data$INJUR_DEATH, YEAR=map_data$YEAR)
...
states$agg2014 <- agg2014$x
states$agg2015 <- agg2015$x
states$agg2016 <- agg2016$x
states$agg2017 <- agg2017$x

### another data navigator function###
output_data <- function(year='All'){
  #prevent null condition
  if (is.null(year)){year='All'}

  if(year=='All'){
    call_index=states$INJUR_DEATH
  }else if (year=='2014'){
    call_index=states$agg2014
  }else if (year=='2015'){
    call_index=states$agg2015
  }else if (year=='2016'){
    call_index=states$agg2016
  }else if (year=='2017'){
    call_index=states$agg2017
  }
  return(call_index)
}
...
shinyServer(function(input, output, session) {
  ...
  output$heatmap <- renderLeaflet({
    ...
    years <- input$year1
    the_index <- output_data(years)
    labels <- sprintf(
      "<strong>%s</strong><br/>%g people",
      states$name, the_index
    ...
    leaflet(states) %>%
      ...
      addLegend(pal = pal, values = ~INJUR_DEATH, opacity = 0.7,
                position = "bottomright", title = sprintf("Total by state at year %s", years))
